...
def alphabeta_full_search(state, game):
    ...
    def max_value(state, alpha, beta, utilities):
        if game.terminal_test(state):
            return game.utility(state, player)
        v = -infinity
        for a in game.actions(state):
            sta = game.result(state, a)
            v = max(v, min_value(sta, alpha, beta, utilities))
    ...
    def min_value(state, alpha, beta, utilities):
        if game.terminal_test(state):
            return game.utility(state, player)
        v = infinity
        for a in game.actions(state):
            sta = game.result(state, a)
            v = min(v, max_value(sta, alpha, beta, utilities))
            utilities.append((a, sta.utility))
            if v <= alpha:
                utilities.append(('\n', '\n'))
                return v
    ...
    def dox(a, game, state, utilities):
        print a
        ret = min_value(game.result(state, a), -infinity, infinity, utilities)
    ...
    ret = argmax(game.actions(state),
                  lambda a: dox(a, game, state, utilities))
    return ret
...
def alphabetafull_player(game, state):
...
play_game(TicTacToe(), alphabetafull_player, query_player)
